...
/* Steps:
 ...
 3. Check sort by option availability
 */
public class TC_REG_MUK_027 extends TestBase {
	...
	public void tc_REG_MUK_025(Hashtable<String, String> data)
			throws IOException {

		test.log(LogStatus.INFO, " Starting test - tc_REG_MUK_025");
		System.out.println(" Starting test - tc_REG_MUK_025");
		...
		if (getElementsSize("plp_Category_Refinement_text_xpath") > 0) {
			...
			if (getElementsSize("plp_premature_text_xpath") > 0) {
				if(getElementsSize("plp_SortBy_dropDown_xpath")>0){
					reportPass("Sort By options are available");
					
				}else{
					reportFailure("sort by option is not available");
				}
			} else {
	...
	public void tearDown() {
		...
		test.log(LogStatus.INFO, " Ending test - tc_REG_MUK_025");
		System.out.println(" Ending test - tc_REG_MUK_025");
