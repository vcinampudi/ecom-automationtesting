...
public class TC_REG_MUK_097 extends TestBase {
	...
	public void tc_REG_MUK_097(Hashtable<String, String> data)
			throws IOException {

		test.log(LogStatus.INFO, " Starting test - tc_REG_MUK_097");
		System.out.println(" Starting test - tc_REG_MUK_097");
		...
		if (getElementsSize("plp_Category_Refinement_text_xpath") > 0) {
			...
			if (getElementsSize("plp_premature_text_xpath") > 0) {
				if (getElementsSize("plp_QuickView_Link_xpath") > 0) {
					List<WebElement> element = driver.findElements(By
							.xpath("//a[contains(text(),'Quick View')]"));
					element.get(1).click();
					wait(3);
					if (getElementsSize("plp_ViewDetailsPopup_xpath") > 0) {
						reportPass("Quick View popup is getting Displayed");
					} else {
						reportFailure("Quick View is not working");
					}

				} else {
					reportFailure("Quick View option is not available");
				}
			} else {
	...
	public void tearDown() {
		...
		test.log(LogStatus.INFO, " Ending test - tc_REG_MUK_097");
		System.out.println(" Ending test - tc_REG_MUK_097");
