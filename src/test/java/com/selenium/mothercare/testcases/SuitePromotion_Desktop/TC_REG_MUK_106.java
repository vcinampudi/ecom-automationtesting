...
5. Compare price before and after applying promo code
*/
public class TC_REG_MUK_106 extends TestBase {
	...
	public void tc_REG_MUK_70(Hashtable<String, String> data) {
		...
		if (getElementsSize("cart_ApplyPromo_Code_Link_xpath") > 0) {
			...
			oldPrice=driver.findElement(By.xpath("//td[.='Subtotal']/following-sibling::td")).getText().substring(1);
			...
			if (getElementsSize("cart_AppliedPromoCode_text_xpath") > 0) {
				if(oldPrice!=driver.findElement(By.xpath("//td[.='Subtotal']/following-sibling::td")).getText().substring(1)){
					reportPass("Price has been successfully updated");
				}else{
					reportFailure("Unable to update the price");
				}
			} else {
